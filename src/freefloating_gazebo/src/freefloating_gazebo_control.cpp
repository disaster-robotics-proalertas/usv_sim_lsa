...
namespace gazebo
{
	...
	bool
	FreeFloatingControlPlugin::SwitchService (std_srvs::EmptyRequest &req, std_srvs::EmptyResponse &res)
	{
	...
	void
	FreeFloatingControlPlugin::Load (physics::ModelPtr _model, sdf::ElementPtr _sdf)
	{
		...
		if (control_body_)
		{
			...
			if (wrench_control_)
			{
				ops = ros::SubscribeOptions::create<geometry_msgs::Wrench> (
				        body_command_topic, 1, boost::bind (&FreeFloatingControlPlugin::BodyCommandCallBack, this, _1),
			...
			else
			{
				ops = ros::SubscribeOptions::create<sensor_msgs::JointState> (
				        ...
				        boost::bind (&FreeFloatingControlPlugin::ThrusterCommandCallBack, this, _1), ros::VoidPtr (),
				        &callback_queue_);
			}
			...
			if (thruster_fixed_idx_.size () && !thruster_steer_idx_.size ())
			{
				...
				for (dir_i = 0; dir_i < 6; ++dir_i)
				{
					...
					for (thr_i = 0; thr_i < thruster_fixed_idx_.size (); ++thr_i)
						thruster_max_effort += thruster_max_command_[thruster_fixed_idx_[thr_i]]
		...
		if (control_joints_ && model_->GetJointCount () != 0)
		{
			...
			ros::SubscribeOptions ops = ros::SubscribeOptions::create<sensor_msgs::JointState> (
			        joint_command_topic, 1, boost::bind (&FreeFloatingControlPlugin::JointCommandCallBack, this, _1),
			        ros::VoidPtr (), &callback_queue_);
				...
				for (i = 0; i < thruster_fixed_idx_.size (); ++i)
				{
					...
					else
					{
						norm_ratio = std::max (norm_ratio,
						                       std::abs (thruster_command_ (i)) / thruster_overWater_max_command_[i]);
					}
				}
	...
	void
	FreeFloatingControlPlugin::ComputePseudoInverse (const Eigen::MatrixXd &_M, Eigen::MatrixXd &_pinv_M)
	{
	...
	void
	FreeFloatingControlPlugin::BodyCommandCallBack (const geometry_msgs::WrenchConstPtr &_msg)
	{
		if (!control_body_)
			return;
	...
	void
	FreeFloatingControlPlugin::ThrusterCommandCallBack (const sensor_msgs::JointStateConstPtr &_msg)
	{
		if (!control_body_)
			return;
