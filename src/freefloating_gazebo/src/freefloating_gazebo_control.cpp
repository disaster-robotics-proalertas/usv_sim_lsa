...
void FreeFloatingControlPlugin::ReadVector3(const std::string &_string,
...
bool FreeFloatingControlPlugin::SwitchService(std_srvs::EmptyRequest &req,
...
void FreeFloatingControlPlugin::Load(physics::ModelPtr _model,
		sdf::ElementPtr _sdf) {
	std::cerr << " ######Starting to load FreeFloating Control Plugin for "
			<< _model->GetName();
	// get model and name
	model_ = _model;
	...
	std::cerr
			<< "\n STARTING TO VERIFY EXISTENCE OF CONTROL BODY AND CONTROL JOINTS!"
			<< endl;
	while (!(control_body_ || control_joints_)) {
		...
		std::cerr
				<< "==========FFControlPlugin -  WAIT INTENTIFING CONTROL! body: "
				<< control_body_ << " joints: " << control_joints_ << endl;
	}
	...
	unsigned int i, j;
	if (control_body_) {
		std::cerr << "\n FREEFLOATING_GAZEBO_CONTROL::" << robot_namespace_
				<< " has control body";
		// read topics
		control_node.param("config/body/command", body_command_topic,
				std::string("body_command"));
		control_node.param("config/body/state", body_state_topic,
		...
		while (sdf_element) {
			if (sdf_element->GetName() == "thruster") {
				...
				if (sdf_element->HasElement("map"))  // fixed
					...
					// register map coefs
					std::stringstream ss(
							sdf_element->Get < std::string > ("map"));
					...
					if (sdf_element->HasElement("name"))
						thruster_names_.push_back(
								sdf_element->Get < std::string > ("name"));
					else {
					...
					ROS_INFO("Adding %s as a fixed thruster",
					...
					thruster_links_.push_back(
							model_->GetLink(
									sdf_element->Get < std::string > ("name")));

					ROS_INFO("Adding %s as a steering thruster",
							thruster_names_[thruster_names_.size() - 1].c_str());
				}

				// register maximum effort
				if (sdf_element->HasElement("effort"))
					thruster_max_command_.push_back(
							sdf_element->Get<double>("effort"));
				else if (sdf_element->HasElement("effortUnderWater"))
					thruster_max_command_.push_back(
							sdf_element->Get<double>("effortUnderWater"));
				else
					thruster_max_command_.push_back(100);

				if (sdf_element->HasElement("effortOverWater"))
					thruster_overWater_max_command_.push_back(
							sdf_element->Get<double>("effortOverWater"));
				else
					thruster_overWater_max_command_.push_back(100);
				LinkSurface* linkSurface = new LinkSurface();
				std::string topic = "/" + robot_namespace_ + "/Surface/"
						+ thruster_names_[thruster_names_.size() - 1];
		...
		if (wrench_control_ && thruster_steer_idx_.size()) {
			ROS_WARN(
					"%s has steering thrusters and cannot be controlled with wrench",
		...
		thruster_use_publisher_ = rosnode_.advertise<sensor_msgs::JointState>(
				"thruster_use", 1);
		...
		if (wrench_control_) {
			ops = ros::SubscribeOptions::create<geometry_msgs::Wrench>(
					...
					boost::bind(&FreeFloatingControlPlugin::BodyCommandCallBack,
							this, _1), ros::VoidPtr(), &callback_queue_);
		} else {
			ops = ros::SubscribeOptions::create<sensor_msgs::JointState>(
					body_command_topic, 1,
					boost::bind(
							&FreeFloatingControlPlugin::ThrusterCommandCallBack,
							this, _1), ros::VoidPtr(), &callback_queue_);
		}
		...
		if (thruster_fixed_idx_.size() && !thruster_steer_idx_.size()) {
			...
			for (dir_i = 0; dir_i < 6; ++dir_i) {
				...
				for (thr_i = 0; thr_i < thruster_fixed_idx_.size(); ++thr_i)
					thruster_max_effort +=
							thruster_max_command_[thruster_fixed_idx_[thr_i]]
									* std::abs(thruster_map_(dir_i, thr_i));
	...
	if (control_joints_ && model_->GetJointCount() != 0) {
		...
		control_node.param("config/joints/command", joint_command_topic,
				std::string("joint_command"));
		control_node.param("config/joints/state", joint_state_topic,
				std::string("joint_state"));

		// initialize subscriber to joint commands
		ros::SubscribeOptions ops = ros::SubscribeOptions::create<
				sensor_msgs::JointState>(joint_command_topic, 1,
				boost::bind(&FreeFloatingControlPlugin::JointCommandCallBack,
						this, _1), ros::VoidPtr(), &callback_queue_);
		...
		if (control_node.hasParam("config/joints/cascaded_position"))
			control_node.getParam("config/joints/cascaded_position",
					cascaded_position);
	...
	update_event_ = event::Events::ConnectWorldUpdateBegin(
			boost::bind(&FreeFloatingControlPlugin::Update, this));

	ros::spinOnce();
	ROS_INFO("######Started FreeFloating Control Plugin for %s.",
	...
	if (controller_is_running_) {
		...
		if (control_joints_ && joint_command_received_) {
			...
			for (unsigned int i = 0; i < joint_command_.name.size(); ++i) {
				...
				idx = std::distance(joint_states_.name.begin(),
		...
		if (control_body_ && body_command_received_) {
			...
			for (i = 0; i < thruster_fixed_idx_.size(); ++i) {
				...
				{
					norm_ratio = std::max(norm_ratio,
							std::abs(thruster_command_(i))
									/ thruster_max_command_[i]);
				}
				else
				{
					norm_ratio = std::max(norm_ratio,
							std::abs(thruster_command_(i))
									/ thruster_overWater_max_command_[i]);
				}
			}
			...
			if (thruster_fixed_idx_.size()) {
				...
				body_->AddForceAtWorldPosition(
						body_->GetWorldPose().rot.RotateVector(
								math::Vector3(fixed(0), fixed(1), fixed(2))),
						body_->GetWorldCoGPose().pos);
				body_->AddRelativeTorque(
						math::Vector3(fixed(3), fixed(4), fixed(5)));
			}
			...
			if (thruster_steer_idx_.size()) {
				for (unsigned int i = 0; i < thruster_steer_idx_.size(); ++i)
					if (body_->GetWorldCoGPose().pos.z	< link_water_surface[i]->waterSurface.z) {
						...
						if (thruster_command_(thruster_steer_idx_[i])
								< -thruster_overWater_max_command_[i]) {
							thruster_links_[i]->AddRelativeForce(
									math::Vector3(0, 0,
											thruster_overWater_max_command_[i]));
						} else if (thruster_command_(thruster_steer_idx_[i])
								> thruster_overWater_max_command_[i]) {
							thruster_links_[i]->AddRelativeForce(
									math::Vector3(0, 0,
											-thruster_overWater_max_command_[i]));
			...
			for (i = 0; i < thruster_fixed_idx_.size(); ++i) {
				if (body_->GetWorldCoGPose().pos.z
						< link_water_surface[i]->waterSurface.z)
					thruster_use_.position[i] = 100
							* std::abs(
									thruster_command_(i)
											/ thruster_max_command_[i]);
				else
					thruster_use_.position[i] =
							100
									* std::abs(
											thruster_command_(i)
													/ thruster_overWater_max_command_[i]);
...
void FreeFloatingControlPlugin::ComputePseudoInverse(const Eigen::MatrixXd &_M,
		Eigen::MatrixXd &_pinv_M) {
	_pinv_M.resize(_M.cols(), _M.rows());
	Eigen::JacobiSVD<Eigen::MatrixXd> svd_M(_M,
...
void FreeFloatingControlPlugin::BodyCommandCallBack(
		const geometry_msgs::WrenchConstPtr &_msg) {
	if (!control_body_)
		return;
...
void FreeFloatingControlPlugin::ThrusterCommandCallBack(
	...
	if (read_effort && (_msg->name.size() != _msg->effort.size())) {
		ROS_WARN(
				"Received inconsistent thruster command, name and effort dimension do not match");
		return;
	} else if (!read_effort && (_msg->name.size() != _msg->position.size())) {
		ROS_WARN(
				"Received inconsistent thruster command, name and position dimension do not match");
		return;
	}
	...
	for (unsigned int i = 0; i < thruster_names_.size(); ++i) {
		for (unsigned int j = 0; j < _msg->name.size(); ++j) {
			if (thruster_names_[i] == _msg->name[j])
				thruster_command_(i) =
						read_effort ? _msg->effort[j] : _msg->position[j];
		}
	}
}
